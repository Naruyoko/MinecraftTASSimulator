...
public class MinecraftTASEditorEditor {
    ...
    private static MinecraftTASEditorInputList actualInputs=null;
    ...
    private static ArrayList<ArrayList<MinecraftTASEditorPlayerInfo>> savedPlayerStates;
    ...
    public static void init() {
        ...
        actualInputs=new MinecraftTASEditorInputList();
        ...
        savedPlayerStates=new ArrayList<ArrayList<MinecraftTASEditorPlayerInfo>>(playerStateSaveSlots);
    ...
    public static void setStartGametype(GameType startGametype) {
        MinecraftTASEditorEditor.startGametype=startGametype;
        setupRunners();
        markEditedAt(-1);
    }
     ...
     */
    public static ArrayList<MinecraftTASEditorPlayerInfo> getSavedPlayerStates(int slot) {
    ...
    public static void savePlayerStatesToSlot(int slot) {
        ...
        savedPlayerStates.set(slot-1,(ArrayList<MinecraftTASEditorPlayerInfo>)simulator.getPlayerStates().clone());
    }
    /**
     ...
     */
    private static float getRotationMult(float mouseSensitivity) {
     ...
     * @return Rotation in degrees per pixel
     */
    private static double getRotationPerPixel(float mouseSensitivity) {
    ...
    private static float changeRotationFromPixels(float before,float mouseSensitivity,int pixels) {
        return (float)((double)before+(double)(pixels*getRotationMult(mouseSensitivity))*0.15D);
    }
    /**
     * Calculate and save actual inputs to actualInputs.
     */
    private static void calculateAccurateInputs() {
        final double ROTATION_PER_PIXEL=getRotationPerPixel(mouseSensitivity);
        final int MAX_SAFE_YAW=900;
        float lastRotationYaw=0F;
        float lastRotationPitch=0F;
        actualInputs=new MinecraftTASEditorInputList();
        for (int i=0;i<getInputLength();i++) {
            MinecraftTASEditorInput input=inputs.get(i,false);
            if (input==null) {
                continue;
            } else {
                if (input.isRotationExact()) {
                    ...
                    if (yawDiff>MAX_SAFE_YAW) {
                        lastRotationYaw=changeRotationFromPixels(lastRotationYaw,mouseSensitivity,MAX_SAFE_YAW);
                    ...
                    if (yawDiff!=0||pitchDiff!=0) {
                        lastRotationYaw=changeRotationFromPixels(lastRotationYaw,mouseSensitivity,yawDiff);
                        lastRotationPitch=changeRotationFromPixels(lastRotationPitch,mouseSensitivity,pitchDiff);
                    }
    ...
    public static void startSimulation() {
        ...
        savedGametype=mc.field_71442_b.func_178889_l();
        simulator.setTarget(tickLength);
        simulator.setCallbackOnFinish(new Runnable() {
            public void run() {
                mc.field_71439_g.func_70634_a(savedPosition.field_72450_a,savedPosition.field_72448_b,savedPosition.field_72449_c);
                ...
                mc.field_71442_b.func_78746_a(savedGametype);
    ...
    public static void saveFile(File file) {
        ...
        try {
            ...
            writer.newLine();
            writer.write("startGametype="+MinecraftTASEditorUtil.stringifyGameType(startGametype));
            writer.newLine();
            ...
            writer.newLine();
            writer.write("editorVersion="+MinecraftTASEditorMod.VERSION);
            writer.newLine();
            writer.newLine();
            writer.write("!input");
            writer.newLine();
        ...
        try {
                ...
                if (line.length()==0||line.charAt(0)=='%') {
                    ...
                    if (StringUtils.isNumeric(argsNoSpace[0])) {
                        ...
                        if (args.length>3) { //Required check for file format version < 2
                            if (args[3].contains("=")) input.setRotationExact(true);
                        }
                    ...
                    if (key.equals("startPosition")) {
                        ...
                        if (!MinecraftTASEditorUtil.isInteger(value)) {
                            MinecraftTASEditorMod.outputErrorToChatAndLog("Parse failed at line "+lineN);
                            readFailed=true;
                            break;
                        }
                        ...
                        newStartGametype=MinecraftTASEditorUtil.toGameType(value);
            ...
            if (!readFailed) {
                if (fileFormatVersionIndicated==0||fileFormatVersionIndicated>FILE_FORMAT_VERSION) {
                    mc.field_71439_g.func_145747_a(new ChatComponentText(EnumChatFormatting.YELLOW+"File format version was not specified or later than the editor handles (Given "+fileFormatVersionIndicated+", expected up to "+FILE_FORMAT_VERSION+"). Simulation may be desynced."));
                }
                if (!editorVersionIndicated.equals(MinecraftTASEditorMod.VERSION)) {
                    mc.field_71439_g.func_145747_a(new ChatComponentText(EnumChatFormatting.YELLOW+"Editor version used was not specified or is different (Given "+editorVersionIndicated+", using "+MinecraftTASEditorMod.VERSION+"). Simulation may be desynced."));
                }
                if (!isRunning) start();
            ...
            mc.field_71439_g.func_71165_d(EnumChatFormatting.RED+"Failed to read from file. Error occurred at line "+lineN);
            e.printStackTrace();
            return;
        }
    }
